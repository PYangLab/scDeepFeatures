...
while(j<=10){
  for(nGroups in c(5, 10, 15, 20)){ # cell type number
    ...
    # read simulated data
    sce = readRDS(glue("../../Data/Simulation/Tabula_sapiens/Setting1/Simulation_setting1_nCells_200_nGroups_{nGroups}__iteration{j}_seeds{n}.rds"))
    ...
    dir.create(glue("../../Result/TS_Fs_marker/Traditional/Setting1"),showWarnings = FALSE)
    dir.create(glue("../../Result/TS_Fs_marker/Traditional/Setting1/{method}"),showWarnings = FALSE)
    saveRDS(res, file = glue("../../Result/TS_Fs_marker/Traditional/Setting1/{method}/Res_{method}_Simulation_setting1_nCells_200_nGroups_{nGroups}__iteration{j}_seeds{n}.rds"))
