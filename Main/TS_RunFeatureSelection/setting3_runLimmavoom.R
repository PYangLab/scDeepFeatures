...
while(j<=10){
  for(nGroups in c(10, 20)){ # cell type number
    for (imbratio in c(1/2, 1/4, 1/10)){
      ...
      # read simulated data
      sce = readRDS(glue("../../Data/Simulation/Tabula_sapiens/Setting3/nGroups_{nGroups}/imbratio_{imbratio}/Simulation_setting3_nCellsMajor_200_nGroups_{nGroups}_iteration{j}_imbalanceRatio_1_{imbratio}.rds"))
      ...
      dir.create(glue("../../Result/TS_Fs_marker/Traditional/Setting3"),showWarnings = FALSE)
      dir.create(glue("../../Result/TS_Fs_marker/Traditional/Setting3/{method}"),showWarnings = FALSE)
      saveRDS(res, file = glue("../../Result/TS_Fs_marker/Traditional/Setting3/{method}/Res_{method}_Simulation_setting3_nGroups_{nGroups}_imbratio_{imbratio}_iteration{j}.rds"))
