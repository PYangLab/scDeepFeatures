...
while(j<=10){
  for(nGroups in c(10,20)){#seq(5,20,5)
    for (nCells in seq(50,250,50)){
      ...
      sce = readRDS(glue("../../Data/Simulation/Tabula_sapiens/Setting2/nGroups_{nGroups}/nCells_{nCells}/Simulation_setting2_nGroups_{nGroups}_nCells_{nCells}_iteration{j}.rds"))
      ...
      dir.create(glue("../../Result/TS_Fs_marker/Traditional/Setting2"),showWarnings = FALSE)
      dir.create(glue("../../Result/TS_Fs_marker/Traditional/Setting2/{method}"),showWarnings = FALSE)
      saveRDS(res, file = glue("../../Result/TS_Fs_marker/Traditional/Setting2/{method}/Res_{method}_Simulation_setting2_nGroups_{nGroups}_nCells_{nCells}_iteration{j}.rds"))
