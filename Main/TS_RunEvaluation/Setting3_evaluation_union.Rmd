  ...
  for(nGroups in c(10,20)){#seq(5,20,5)
    for (imbratio in c(1/2, 1/4, 1/10)){
      ...
      for (method in nn_methods){
        ...
        file_list = list.files(glue("../../Result/TS_Fs_marker/Nn/Setting3/{method}/nGroups_{nGroups}_imb_{imbratio}_iteration{j}/"))
        for (topN in c(5, 10, 20)) { #5, 10, 20, 50, 100
          ...
          for (anchor_celltype_num in (c(1:length(cellTypes))-1)){
            ...
            res_fs = read_csv(glue("../../Result/TS_Fs_marker/Nn/Setting3/{method}/nGroups_{nGroups}_imb_{imbratio}_iteration{j}/{file}"), show_col_types = FALSE)
          ...
          print("Accuracy is:")
          print(res[[1]][,1][1])
          dir.create(glue("../../Result/TS_Classification/KKnn/Setting3/top{topN}"), showWarnings = FALSE)
          dir.create(glue("../../Result/TS_Classification/KKnn/Setting3/top{topN}/{method}"), showWarnings = FALSE)
          saveRDS(res, file = glue("../../Result/TS_Classification/KKnn/Setting3/top{topN}/{method}/KKnn_top{topN}_result_nGroups_{nGroups}_imb_{imbratio}_iteration{j}.rds"))
          }
        }
  ...
  for(nGroups in c(10,20)){#seq(5,20,5)
    ...
    cellTypes = levels(as.factor(train.rna$cellTypes))
      ...
      for (method in trad_methods){
        res_fs = readRDS(glue("../../Result/TS_Fs_marker/Traditional/Setting3/{method}/Res_{method}_Simulation_setting3_nGroups_{nGroups}_imbratio_{imbratio}_iteration{j}.rds"))
        for (topN in c(5,10,20)){ #5, 10, 20, 50, 100
          union_genes = c()
          for (anchor_celltype_num in (c(1:length(cellTypes))-1)){
              anchor_celltype = cellTypes[anchor_celltype_num+1]
          ...
          union_genes = unique(union_genes)
          union_genes <- union_genes[!grepl("^NA$|^NA\\.", union_genes)]
          tmp_sce = train.rna[union_genes,]
          set.seed(1)
          res = doKKnn_simple_union(tmp_sce, tmp_sce$cellTypes, test.rna[union_genes,],  test.cty)
          cat("finished KKnn")
    ...
    for (imbratio in c(1/2, 1/4, 1/10)){
      ...
      for (method in nn_methods){
        ...
        for (topN in c(5, 10, 20)) { #5, 10, 20, 50, 100
          ...
          print("Accuracy is:")
          print(res[[1]][,1][1])
          dir.create(glue("../../Result/TS_Classification/SVM/Setting3/top{topN}"), showWarnings = FALSE)
          dir.create(glue("../../Result/TS_Classification/SVM/Setting3/top{topN}/{method}"), showWarnings = FALSE)
          saveRDS(res, file = glue("../../Result/TS_Classification/SVM/Setting3/top{topN}/{method}/SVM_top{topN}_result_nGroups_{nGroups}_imb_{imbratio}_iteration{j}.rds"))
          }
        }
    cat(glue("==================finished {n} datasets================= \n"))
    n=n+1
