  ...
  while(j<=10){
    for(nGroups in c(10,20)){#seq(5,20,5)
      for (nCells in seq(50,250,50)){
        ...
        tmp_res <- c(tmp_mean[[1]],tmp_mean[[2]],tmp_mean[[7]], nGroups, nCells, j, method, "traditional")
        df <- rbind(df, tmp_res)
      }
    }
    j=j+1
...
df <- as.data.frame(df)
colnames(df) <- c("Sensitivity", "Specificity", "F1", "nGroups", "nCells", "Iteration", "Method", "Category")
...
dl_cat <- c(rep("Deep learning - gradient based", 3), rep("Deep learning - perturbation", 4))
dl_cat_num = 1
df <- data.frame()
  ...
  while(j<=10){
    for(nGroups in c(10,20)){#seq(5,20,5)
      for (nCells in seq(50,250,50)){
        ...
        tmp[[2]][is.na(tmp[[2]])] = 0
        tmp_mean  = rowMeans(tmp[[2]])
        tmp_res <- c(tmp_mean[[1]],tmp_mean[[2]],tmp_mean[[7]], nGroups, nCells, j, method, dl_cat[dl_cat_num])
        df <- rbind(df, tmp_res)
      }
    }
    j=j+1
...
ggsave(p1, width = 5, height=4, filename = glue("../../Figures/TS_Setting2/Classification/{classification_method}_lineplot_F1_only_topN{topN}.pdf"))
p1
```
...
p1
ggsave(p1, width = 4, height = 4, filename = glue("../../Figures/TS_Setting2/Classification/{classification_method}_lineplot_coef_top{topN}.pdf"))
```
...
p1 = ggplot(aes(x=method, y=mean,fill=variable),data = df.bar[df.bar$variable %in% c("Sensitivity", "Specificity"),]) + geom_bar(position=position_dodge(),stat="identity") + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,position=position_dodge(.9)) + facet_grid(nCells~nGroups) +  coord_cartesian(ylim = c(0.4, 1)) +theme_light() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

ggsave(p1, height = 5, width = 9, filename = glue("../../Figures/TS_Setting2/Classification/{classification_method}_Setting2_barplot_top{topN}_sen_spe.pdf"))
