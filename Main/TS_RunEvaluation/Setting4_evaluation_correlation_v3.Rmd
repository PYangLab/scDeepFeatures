...
####################
n=1 # seed number 1
sce = readRDS(glue("../../Data/Simulation/Tabula_sapiens//Setting4/Simulation_setting4_seed_{n}_iteration1.rds"))
...
for (method in nn_methods){
  for (anchor_celltype_num in (c(1:length(cellTypes))-1)){
    ...
    for(j in c(1:9)){
      x = x[! x %in% j]
      for(i in x){
        file_list = list.files(glue("../../Result/TS_Fs_marker/Nn/Setting4/{method}/seed_{n}_iteration{j}_all/"))
        file = file_list[grep(anchor_celltype, file_list)]
        res_fs1 = suppressMessages(read_csv(glue("../../Result/TS_Fs_marker/Nn/Setting4/{method}/seed_{n}_iteration{j}_all/{file}"), show_col_types = FALSE))
        
      
        file_list = list.files(glue("../../Result/TS_Fs_marker/Nn/Setting4/{method}/seed_{n}_iteration{i}_all/"))
        file = file_list[grep(anchor_celltype, file_list)]
        res_fs2 = suppressMessages(read_csv(glue("../../Result/TS_Fs_marker/Nn/Setting4/{method}/seed_{n}_iteration{i}_all/{file}"), show_col_types = FALSE))
...
df.nn = df
saveRDS(df, file = glue("../../Result/TS_Reproducibility/seed_{n}_nn_pearson_correlation.rds"))
```
...
for (method in trad_methods){
  for (anchor_celltype_num in (c(1:length(cellTypes))-1)){
    ...
    for(j in c(1:9)){
      x = x[! x %in% j]
      for(i in x){
        res_fs = readRDS(glue("../../Result/TS_Fs_marker/Traditional/Setting4/{method}/Res_{method}_Simulation_setting4_seed_{n}_iteration{j}_all.rds"))
        tmp_res_fs1 = res_fs$result[[anchor_celltype]]
        
        res_fs = readRDS(glue("../../Result/TS_Fs_marker/Traditional/Setting4/{method}/Res_{method}_Simulation_setting4_seed_{n}_iteration{i}_all.rds"))
...
df$method <- as.factor(df$method)
df.trad = df
saveRDS(df.trad, file = glue("../../Result/TS_Reproducibility/seed_{n}_trad_pearson_correlation.rds"))
```
...
ggsave(p1, height = 5, width = 6, filename = "../../Figures/TS_Setting4/averaged_pearson_correlation.pdf")
p1
```
