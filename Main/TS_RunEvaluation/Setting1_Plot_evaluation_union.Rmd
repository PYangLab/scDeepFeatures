...
# Classification
```{r}
classification_method = "KKnn" # KKnn 
```
...
p1
ggsave(p1,height = 5, width = 7, filename = glue("../../Figures/TS_Setting1/Classification/{classification_method}_Setting1_boxplot_top{topN}_F1.pdf"))
```
...
for (nGroup in c(5,10,15,20)){
  ...
  for (method in nn_methods){
    ...
    y = df[df$nGroups == nGroup & df$Method=="11_runScDD",]$F1
    test.res = wilcox.test(x, y, paired = TRUE, alternative = "greater")
    print(glue("p-val of {method} larger than MAST is {test.res$p.value}"))
  }
}
```
