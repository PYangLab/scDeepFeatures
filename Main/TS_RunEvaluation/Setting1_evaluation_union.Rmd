...
j=1 # repeat times
while(j<=10){
  for(nGroups in c(5,10,15,20)){ # cell type number 5:20
  # read sce
  ...
  for (method in nn_methods){
    ...
    file_list = list.files(glue("../../Result/TS_Fs_marker/Nn/Setting1/{method}/dataset_seeds_{n}/"))
    for (topN in c(5, 10, 20)) { #5, 10, 20, 50, 100
      ...
      for (anchor_celltype_num in (c(1:length(cellTypes))-1)){
        ...
        res_fs = read_csv(glue("../../Result/TS_Fs_marker/Nn/Setting1/{method}/dataset_seeds_{n}/{file}"), show_col_types = FALSE)
      ...
      print("Accuracy is:")
      print(res[[1]][,1][1])
      dir.create(glue("../../Result/TS_Classification/KKnn/Setting1/top{topN}"), showWarnings = FALSE)
      dir.create(glue("../../Result/TS_Classification/KKnn/Setting1/top{topN}/{method}"), showWarnings = FALSE)
      dir.create(glue("../../Result/TS_Classification/KKnn/Setting1/top{topN}/{method}/dataset_seeds_{n}"), showWarnings = FALSE)
      saveRDS(res, file = glue("../../Result/TS_Classification/KKnn/Setting1/top{topN}/{method}/dataset_seeds_{n}/KKnn_top{topN}_result_nGroups_{nGroups}_iteration{j}_seeds{n}_method_{method}.rds"))
      }
...
while(j<=10){
  ...
  cellTypes = levels(as.factor(train.rna$cellTypes))
    ...
    for (method in trad_methods){
      res_fs = readRDS(glue("../../Result/TS_Fs_marker/Traditional/Setting1/{method}/Res_{method}_Simulation_setting1_nCells_200_nGroups_{nGroups}__iteration{j}_seeds{n}.rds"))
      for (topN in c(5,10,20)){ #5, 10, 20, 50, 100
        union_genes = c()
        for (anchor_celltype_num in (c(1:length(cellTypes))-1)){
            anchor_celltype = cellTypes[anchor_celltype_num+1]
...
while(j<=10){
  ...
  for (method in nn_methods){
    ...
    for (topN in c(5, 10, 20)) { #5, 10, 20, 50, 100
      ...
      print("Accuracy is:")
      print(res[[1]][,1][1])
      dir.create(glue("../../Result/TS_Classification/SVM/Setting1/top{topN}"), showWarnings = FALSE)
      dir.create(glue("../../Result/TS_Classification/SVM/Setting1/top{topN}/{method}"), showWarnings = FALSE)
      dir.create(glue("../../Result/TS_Classification/SVM/Setting1/top{topN}/{method}/dataset_seeds_{n}"), showWarnings = FALSE)
      saveRDS(res, file = glue("../../Result/TS_Classification/SVM/Setting1/top{topN}/{method}/dataset_seeds_{n}/KKnn_top{topN}_result_nGroups_{nGroups}_iteration{j}_seeds{n}_method_{method}.rds"))
      }
