...
df.line
df.line = df.line[df.line$variable %in% c("F1"),]
df.line$variable <- factor(df.line$variable, levels = c("F1","Senstivity","Specificity"))
p1 = ggplot(aes(x=nGroups, y=value, group=Method), data=df.line)+geom_line(aes(color=Method))+geom_point(aes(color=Method))+coord_cartesian(ylim=c(0.7,1))+theme_classic()
ggsave(p1, width = 6, height = 3.5, filename = glue("../../Figures/Setting1/Classification/{classification_method}_top{topN}_lineplot.pdf"))
p1
```
...
p1 = ggplot(aes(x=method, y=coef, fill=as.factor(sig)), data = df.lm2) + geom_bar(stat="identity") + coord_flip() + scale_fill_manual(values=c("grey", "red")) + theme_classic()
ggsave(p1, width = 6, height = 3.5, filename = glue("../../Figures/Setting1/Classification/{classification_method}_top{topN}_lineplot_coef.pdf"))
p1
```
...
p1 = ggplot(aes(x=method, y=mean,fill=variable),data = df.bar[df.bar$variable %in% c("Senstivity", "Specificity"),]) + geom_bar(position=position_dodge(),stat="identity") + geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,position=position_dodge(.9)) + facet_wrap(nGroups~.) +  coord_cartesian(ylim = c(0.75, 1)) +theme_light() +theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))

ggsave(p1, height = 5, width = 7, filename = glue("../../Figures/Setting1/Classification/{classification_method}_Setting1_barplot_top{topN}_sen_spe.pdf"))
